...
function clearSessionCookie(res: ServerResponse): void {
  res.setHeader('Set-Cookie', `${getSessionCookieName()}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0`);
}
...
export async function handleApi(
  ...
  if (urlObj.pathname === '/api/logout' && req.method === 'POST') {
    const cookie = (req.headers.cookie ?? '').split(';').find((c) => c.trim().startsWith(`${getSessionCookieName()}=`));
  ...
  if (urlObj.pathname === '/api/me' && req.method === 'GET') {
    const uid = req.userId ?? (req.headers['x-user-id'] as string);
    if (!uid || uid === 'local') {
      if (!auth.authAvailable()) return void sendJson(res, 200, { user: null });
      return void sendJson(res, 401, { error: 'Not authenticated.' });
    }
  ...
  if (urlObj.pathname === '/api/profile' && req.method === 'GET') {
    const uid = req.userId ?? (req.headers['x-user-id'] as string);
    if (!uid || uid === 'local') {
      if (!auth.authAvailable()) return void sendJson(res, 200, { profile: null });
      return void sendJson(res, 401, { error: 'Not authenticated.' });
    }
    const user = await auth.getUserById(uid);
    if (!user) return void sendJson(res, 401, { error: 'User not found.' });
  ...
  const userId = req.userId ?? (req.headers['x-user-id'] as string) ?? 'local';
