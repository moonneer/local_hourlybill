...
import { Play, Plus, Save, FileEdit, Trash2, StopCircle, ArrowRight, Activity, Calendar, Tag, RefreshCw } from "lucide-react";
...
export default function QueryBuilderPage() {
  ...
  const runPipeline = () => {
    ...
    const url = apiUrl(`/api/run-pipeline?query=${encodeURIComponent(queryName)}`);
  ...
  return (
    <div className="max-w-[1000px] mx-auto w-full min-w-0 p-3 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-24">
      {/* Header Controls */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">Matters</h1>
          <p className="text-sm sm:text-base text-muted-foreground mt-1">Configure email analysis and matter details.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto min-w-0">
          <Select value={isNew ? "" : selectedQuery} onValueChange={handleQueryChange}>
            <SelectTrigger className="w-full sm:w-[260px] min-w-0 bg-white shadow-sm border-border/80 h-11 sm:h-10 touch-manipulation">
