  ...
  return (
    <header className="flex items-center justify-between gap-4 px-6 py-4 border-b border-border bg-white/80 backdrop-blur-md sticky top-0 z-30 shrink-0 shadow-sm">
      <div className="flex items-center gap-4 min-w-0">
        <SidebarTrigger data-testid="button-sidebar-toggle" className="text-muted-foreground hover:text-foreground shrink-0" />
        <div className="h-4 w-px bg-border hidden sm:block shrink-0" />
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-primary text-primary-foreground shadow-sm shrink-0">
            <Scale className="w-4 h-4" />
          </div>
          <div className="flex items-center gap-2 text-sm min-w-0">
            <span className="font-bold tracking-tight text-foreground truncate">Hourly Bill</span>
            <ChevronRight className="w-4 h-4 text-muted-foreground/50 shrink-0" />
            <span className="font-semibold text-foreground tracking-tight truncate capitalize">{title}</span>
          </div>
        </div>
      </div>
      <ProfileMenu />
        ...
        <SidebarProvider style={style as React.CSSProperties}>
          <div className="flex h-screen w-full overflow-hidden bg-background text-foreground selection:bg-primary/20 selection:text-primary">
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden bg-background">
              <Header />
              <main className="flex flex-1 flex-col min-h-0 overflow-auto bg-slate-50/50">
                <Router />
              </main>
            </div>
          </div>
